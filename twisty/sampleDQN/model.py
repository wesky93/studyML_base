...
class DQN :
    ...
    def __init__( self, n_action, n_size, state ) :
        ...
        self.state_t = np.stack( (state, state, state, state, state, state, state, state), axis=1 )[ 0 ]
        # 현재 상태만으로 학습하도록 변경
        ...
        self.input_state = tf.placeholder( tf.float32, [ None, len( self.state_t ), self.n_size ] )
    ...
    def build_model( self ) :
        ...
        n_input = len( self.state_t ) * self.n_size
        # ??: -1이 뭘까? 0으로 바꿔도 되지 않을까?
