...
### 결과
- 평균 완성확률은 60%정도 이며 완성된 큐브의 평균 회전수는 3위아래로 아주 성공적으로 학습됬다

### 기타
- 다음은 이번 랩의 가중치를 기반으로 스크램길이 10으로 학습을 해봐야 겠다
...
### 결과
- 완성된 큐브의 평균 회전수가 lab11과 유사한것으로 봐서는 스크램블 10이상이 제대로 학습되지 않는것으로 생각된다. 

### 기타
- 단계가 너무 확 뛰어서 그럴수도 있기에 일단 가중치를 완전 초기화 시켜서 학습해보고 제대로 안되다면 스크램블 길이를 5로 낮추어 다시 단계별 학습을 해봐야 할듯 하다.
- 또한 스크램블길이가 3에서 위아래인 이유가 스크램블된 큐브가 복잡하지 않은것 같으므로 앞으로는 스크램블된 큐브에 완성된 면 갯수가 2개 이하가 되도록 한다.
...
## lab13_2017025
### 목표
- 이전 학습을 사용하지 않고 스크램블 길이 10짜리를 학습하기
